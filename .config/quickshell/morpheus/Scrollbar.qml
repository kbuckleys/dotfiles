...
Item {
  ...
  // How far one wheel notch over the BAR travels. 0 keeps the default
  // quarter of a viewport, which is right for a list of lines. A view made
  // of tiles wants its own number — a notch that does not land on a row
  // boundary leaves every row half cut off — so it says so: see Picasso,
  // where this and its ElasticScroll.step are both one row.
