...
PanelWindow {
  ...
  Item {
    ...
    // Behind the panel as a sibling, never inside it: bg clips, and a
    // panel that clips would clip its own shadow away. Suppressed while
    // morphed, when this IS the pill and the bar casts for it.
    LayerShadow {
      panel: bg
      cornerRadius: popup.outerRadius
